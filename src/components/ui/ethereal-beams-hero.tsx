...
const DirLight: FC<{ position: [number, number, number]; color: string }> = ({ position, color }) => {
  ...
  return <directionalLight ref={dir} position={position} intensity={8} color={color} />;
};
...
const Beams: FC<BeamsProps> = ({
  ...
  return (
    <group rotation={[0, 0, degToRad(rotation)]}>
      ...
      <DirLight position={[0, 5, 10]} color={lightColor} />
      <DirLight position={[0, 5, -10]} color={lightColor} />
    </group>
  );
};
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      <Canvas
        ...
        dpr={[1, 2]}
      >
        <Beams lightColor={lightColor} speed={speed} rotation={rotation} />
      </Canvas>
    </div>
  );
}
